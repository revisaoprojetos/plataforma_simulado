...
export const BLOCKS: BlockMeta[] = [
  ...
  { type: 'diag-pilares', title: 'Diagnóstico — Pilares', icon: LayoutGrid, category: 'avaliacao', dynamic: true, supportsVars: true,
    defaults: {
      ...
      corFundo: '#fef3d6', fitaCor: '#3b5bdb', fitaAltura: 4, bordaRaio: 4, padding: 12, gap: 16, divisoria: true, divisoriaCor: '#cbb26b',
    ...
    case 'gabarito-grid': {
      ...
      if (origem === 'ambos') {
        ...
        if (orientacao === 'horizontal') {
          // Tabela com questões em COLUNAS: faixas com linhas Nº / Alternativa marcada / Gabarito.
          ...
          const rot = { ...cellBase, textAlign: 'left' as const, width: 118, minWidth: 118, fontSize: 8.5, fontWeight: 800 as const, lineHeight: 1.15, background: corHeader, color: corHeaderTexto }
          return (
                ...
                <table key={fi} style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', border: `1px solid ${borda}` }}>
                  <tbody>
                    <tr style={{ background: corHeader, color: corHeaderTexto }}>
                      <td style={rot}>Nº</td>
                      {faixa.map((n, idx) => <td key={n} style={{ ...cellBase, fontWeight: 800, fontSize: 10, borderLeft: `1px solid ${borda}`, borderRight: idx === faixa.length - 1 ? 'none' : cellBase.borderRight }}>{n}</td>)}
                    </tr>
                    <tr style={{ borderTop: `1px solid ${borda}` }}>
                      <td style={rot}>Alternativa marcada</td>
                      {faixa.map((n, idx) => { const mk = marcadaDe(n); return <td key={n} style={{ ...cellBase, fontWeight: 800, color: corMkDe(mk, oficialDe(n)), borderLeft: `1px solid ${borda}`, borderRight: idx === faixa.length - 1 ? 'none' : cellBase.borderRight }}>{mk}</td> })}
                    </tr>
                    <tr style={{ borderTop: `1px solid ${borda}` }}>
                      <td style={rot}>Gabarito</td>
                      {faixa.map((n, idx) => <td key={n} style={{ ...cellBase, fontWeight: 800, color: corHeader, borderLeft: `1px solid ${borda}`, borderRight: idx === faixa.length - 1 ? 'none' : cellBase.borderRight }}>{oficialDe(n)}</td>)}
                    </tr>
                  </tbody>
                </table>
              ))}

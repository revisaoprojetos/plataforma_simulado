...
function htmlDiagnostico(item: ItemCaderno, vars: Record<string, string>, disc: DiscBanco[]): string {
  ...
  if (discs.length) {
    ...
    for (const d of discs) {
      ...
      // Gabaritou (sem assuntos errados) com dados reais → nada; sem dados (modelo) → placeholder ilustrativo.
      const asHtml = assuntos.length ? assuntos.map((x) => `<div style="font-size:${fsz(`disc:${d.chave}`, corpo)};color:#5a5570;font-style:italic">- ${esc(x)}</div>`).join('') : (Object.keys(vars).length ? '' : `<div style="font-size:${fsz(`disc:${d.chave}`, corpo)};color:#5a5570;font-style:italic">- Assuntos das questões erradas</div>`)

...
function htmlDiagnostico(item: ItemCaderno, vars: Record<string, string>, disc: DiscBanco[]): string {
  ...
  let h = ''
  if (a.mostrarCabecalho) { const cor = corP('diag_cab', prim); h += `<div style="background:${cor};color:${corTextoP('diag_cab', '#fff')};padding:14px 18px;margin-bottom:12px;${fonteCss('diag_cab')}"><div style="font-size:${fsz('diag_cab_titulo', 22)};font-weight:800;text-align:${alignP('diag_cab_titulo', 'left')};color:${corTextoP('diag_cab_titulo', '#fff')};${estCss('diag_cab_titulo')}${fonteCss('diag_cab_titulo')}">${V(c.tituloCabecalho ?? 'Diagnóstico de Desempenho')}</div>${c.subtitulo ? `<div style="font-size:${fsz('diag_cab_sub', 12)};opacity:.85;margin-top:2px;text-align:${alignP('diag_cab_sub', 'left')};color:${corTextoP('diag_cab_sub', '#fff')};${estCss('diag_cab_sub')}${fonteCss('diag_cab_sub')}">${V(c.subtitulo)}</div>` : ''}</div>` }
  ...
  if (c.gabaritoObs.length || c.gabaritoIntro.length) {
    ...
    if (c.gabaritoObs.length) { const cor = corP('gab_obs', '#a32d2d'); h += `<div style="background:#f5f3ff;border-top:3px solid ${cor};padding:6px 10px">${c.gabaritoObs.map((o) => `<div style="font-size:${fsz('gab_obs', corpo)};color:#5a5570">${V(o)}</div>`).join('')}</div>` }
  }
  return h
}

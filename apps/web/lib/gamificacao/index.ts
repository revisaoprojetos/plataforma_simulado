...
export async function onSimuladoFinalizado(
  ...
  try {
    ...
    const xp = (r.base ?? 0) + (r.por_acerto ?? 0) * (acertos ?? 0) + bonusNota
    await awardXp(svc, { tenantId, estudanteId, origem: 'simulado', refId: sessaoId, xp, meta: { nota, acertos, total } })
...
export async function onPraticaRespondida(
  ...
  try {
    ...
    if (correta) {
      ...
      await awardXp(svc, { tenantId, estudanteId, origem: 'pratica', refId: respostaId, xp: (r.por_acerto ?? 0) + bonus, meta: { disciplinaId, bonus } })

...
export async function GET() {
  ...
  try {
    ...
    const [{ data: evs }, { data: cache }] = await Promise.all([
      svc.from('simulado_xp_eventos')
        ...
        .limit(50),
      svc.from('simulado_gamificacao_estudante').select('xp_total').eq('tenant_id', sessao.tenantId).eq('estudante_id', sessao.estudanteId).maybeSingle(),
    ])
    ...
    return NextResponse.json({ ok: true, eventos, xpTotal: cache?.xp_total ?? 0, curva: config.nivel_curva })
  } catch {

...
export default async function AlunoHome() {
  ...
  if (ids.length) {
    const [{ data: s }, { data: sess }] = await Promise.all([
      svc.from('simulado_simulados').select('id, titulo, status, embed_token, regras, modo_aplicacao, data_inicio, data_fim, created_at').in('id', ids).eq('deletado', false),
      ...
      {disponiveis.length > 0 && (
        <section className="space-y-3">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {disponiveis.slice(0, 8).map((s) => <CardSimulado key={s.id} s={s} />)}
          </div>
        </section>
      )}
    </div>

...
export default async function ResultadoImprimirPage({ params }: { params: Promise<{ st: string }> }) {
  ...
  const [{ data: simulado }, { data: estudante }, { count: totalParticipantes }] = await Promise.all([
    ...
    svc.from('simulado_sessoes_prova').select('*', { count: 'exact', head: true }).eq('simulado_id', sessao.simulado_id).eq('is_teste', false).eq('status', 'finalizada'),
  ])
  ...
  const { data: sq } = await svc
    ...
    .select('ordem, questoes:simulado_questoes(id, enunciado, disciplinas:simulado_disciplinas(nome), alternativas:simulado_alternativas(id, texto, ordem, correta))')

...
export default async function NpsPage() {
  ...
  let semTabela = false
  try {
    avals = await fetchAll<Av>(() => svc.from('simulado_avaliacoes').select('nps, comentario, criado_em, simulado_id, estudante_id').eq('tenant_id', tid).order('criado_em', { ascending: false }))
  ...
  const nps = total ? Math.round(((promotores - detratores) / total) * 100) : null
  const media = total ? (avals.reduce((s, a) => s + a.nps, 0) / total) : null

  // Nome dos simulados (para os comentários).
  const simIds = [...new Set(avals.map((a) => a.simulado_id))]
  const { data: sims } = simIds.length ? await svc.from('simulado_simulados').select('id, titulo').in('id', simIds.slice(0, 200)) : { data: [] as any[] }
  const nomeSim = new Map<string, string>((sims ?? []).map((s: any) => [s.id, s.titulo]))
  const comentarios = avals.filter((a) => (a.comentario ?? '').trim()).slice(0, 40)
                ...
                {comentarios.map((c, i) => (
                  <li key={i} className="flex items-start gap-3 p-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-[11px] text-muted-foreground">{nomeSim.get(c.simulado_id) ?? 'Simulado'} · {formatBrt(c.criado_em)}</p>
                    </div>
                  </li>
                ))}
              </ul>

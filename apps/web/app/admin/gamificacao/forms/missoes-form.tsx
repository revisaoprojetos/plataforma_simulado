...
export function MissoesForm({ config, podeGerenciar }: { config: GamConfig; podeGerenciar: boolean }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <p className="text-sm text-muted-foreground">Missões <strong>diárias</strong> (renovam à meia-noite no fuso do tenant). O aluno ganha o XP ao atingir a meta.</p>

      <div className="space-y-2">
        {missoes.map((m, i) => (
          <Card key={m.id} className="flex flex-wrap items-end gap-3 p-3">
            ...
            <label className="space-y-1">
              <span className="block text-[11px] text-muted-foreground">Tipo</span>
      ...
      {podeGerenciar && (
        <div className="flex items-center gap-2">
          <Button type="button" variant="outline" onClick={add}><Plus className="h-4 w-4" /> Adicionar missão</Button>

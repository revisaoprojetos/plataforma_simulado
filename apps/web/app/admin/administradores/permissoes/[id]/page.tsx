...
import { usuariosDoPerfil } from '@/app/admin/rbac/actions'
...
export default async function CargoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Permissões deste cargo (resumo por módulo).
    ...
    <div className="space-y-5">
      <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
        <div className="flex flex-wrap items-center gap-4 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent p-5">
          <Link href="/admin/administradores/permissoes" className={cn(buttonVariants({ variant: 'ghost', size: 'icon-sm' }), 'shrink-0')}>
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-muted-foreground">{role.descricao || `Cargo ${role.nome}`}</p>
          </div>
          <Link href={`/admin/administradores/permissoes?perfil=${role.id}`} className={cn(buttonVariants({ variant: 'outline', size: 'sm' }), 'ml-auto')}>
      ...
      <div className="rounded-2xl border bg-card p-4">
        <div className="mb-2 flex items-center gap-2 text-sm font-semibold"><KeyRound className="h-4 w-4 text-primary" /> Permissões deste cargo</div>
        {porModulo.size === 0 ? (

...
export default async function PermissoesPage({ searchParams }: { searchParams: Promise<{ perfil?: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">Cada card é um cargo — clique para gerenciar quem o possui; use “Editar permissões” para ajustar a matriz.</p>
      ...
      {data && (
        <details open={!!destaqueId} className="group rounded-2xl border bg-card [&_summary::-webkit-details-marker]:hidden">
          <summary className="flex cursor-pointer list-none items-center gap-2 px-4 py-3">
            ...
            <span className="text-sm font-semibold">Editar permissões (matriz cargos × permissões)</span>
          ...
          <Card className="overflow-hidden rounded-none border-0 border-t" style={{ ['--card-spacing' as any]: '0px' }}>
            <SecaoHeader icon={Shield} titulo="Matriz de Permissões" subtitulo="Cada coluna é um cargo, cada linha uma permissão — clique para ativar/desativar." />

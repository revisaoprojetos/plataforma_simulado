...
export async function listarTaxonomia(tipo: TipoTaxonomia): Promise<{ ok: boolean; itens?: ItemTax[]; error?: string }> {
  ...
  if (cfg.kind === 'entidade') {
    const { data: rows, error } = await svc.from(cfg.tabela).select('id, nome').eq('tenant_id', g.tenantId).order('nome')
    if (error) return { ok: false, error: error.message }
    const qCount = new Map<string, number>()
    const qs = await fetchAll<any>(() => svc.from('simulado_questoes').select(cfg.fk).eq('tenant_id', g.tenantId).eq('deletado', false).not(cfg.fk, 'is', null))
    for (const r of qs) { const id = r[cfg.fk]; if (id) qCount.set(id, (qCount.get(id) ?? 0) + 1) }
    const extraCount = new Map<string, number>()
    if (cfg.extra) {
      const ex = await fetchAll<any>(() => svc.from(cfg.extra!.tabela).select(cfg.extra!.col).eq('tenant_id', g.tenantId).not(cfg.extra!.col, 'is', null))
      for (const r of ex) { const id = r[cfg.extra!.col]; if (id) extraCount.set(id, (extraCount.get(id) ?? 0) + 1) }
    }
    return { ok: true, itens: (rows ?? []).map((d: any) => ({ id: d.id, nome: d.nome, questoes: qCount.get(d.id) ?? 0, extra: cfg.extra ? (extraCount.get(d.id) ?? 0) : undefined })) }
  }

  // Valor: distinct dos valores da coluna (com contagem).
  const rows = await fetchAll<any>(() => svc.from('simulado_questoes').select(cfg.col).eq('tenant_id', g.tenantId).eq('deletado', false).not(cfg.col, 'is', null))
...
export async function unificarTaxonomia(tipo: TipoTaxonomia, canonicaId: string, duplicadaIds: string[]): Promise<{ ok: boolean; error?: string; questoes?: number; removidas?: number; mantida?: string }> {
  if (tipo === 'disciplina') return unificarDisciplinas(canonicaId, duplicadaIds)

...
import { useCriar, useGuardStep, type LinkDraft } from '../criar-context'
...
export default function LinksPage() {
  ...
  // Pares (disciplina, aula) usados nas metas — candidatos a ter link.
  ...
  function puxarDasMetas() {
    ...
    const novos: LinkDraft[] = faltando.map((p) => ({
  ...
  function adicionarManual() {
    ...
    patch({
      links: [...draft.links, { disciplina: novaDisc.trim(), disciplina_id: novaDiscId, aula: novaAula.trim(), tema: '', urls: {} }],
    })
    setNovaAula('')
  }
  ...
  return (
    <Etapa
      ...
      descricao="Para cada (disciplina, aula) você define o tema e o link em cada plataforma. O link casa por texto exato com a aula da meta — “01” e “1” são diferentes."
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground">
          {draft.links.length.toLocaleString('pt-BR')} link(s) · etapa opcional
        </span>
      </div>
      ...
      {draft.links.length === 0 ? (
        <div className="rounded-2xl border bg-card py-12 text-center text-sm text-muted-foreground shadow-sm">
          <Link2 className="mx-auto mb-2 h-7 w-7 text-muted-foreground/40" />
          Nenhum link ainda. Puxe as aulas das metas ou adicione manualmente. Você também pode fazer isso depois.
        </div>
      ) : (
        <div className="space-y-3">
          {draft.links.map((l, i) => (
            <div key={`${chaveDe(l.disciplina, l.aula)}-${i}`} className="rounded-2xl border bg-card p-4 shadow-sm">
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm font-semibold">
                  {l.disciplina} <span className="font-mono font-normal text-muted-foreground">· aula {l.aula}</span>
                </p>
                <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => remover(i)} title="Remover">
                  <Trash2 className="h-3.5 w-3.5 text-destructive" />
                </Button>
              </div>
              <div className="mt-2 space-y-1.5">
                <Input value={l.tema} onChange={(e) => atualizar(i, { tema: e.target.value })} placeholder="Tema (aparece no desempenho do aluno)" className="h-8" />
                <div className="grid gap-1.5 sm:grid-cols-2">
                  {plataformas.map((p) => (
                    <div key={p.slug} className="flex items-center gap-2">
                      <span className="w-16 shrink-0 truncate text-xs font-medium text-muted-foreground" title={p.nome}>
                        {p.nome}
                      </span>
                      <Input
                        value={l.urls[p.slug] ?? ''}
                        onChange={(e) => setUrl(i, p.slug, e.target.value)}
                        placeholder="https://…"
                        className="h-8"
                      />
                    </div>

...
import { GRIFOS, GRIFOS_LISTA, corDoGrifo, type TipoGrifo } from '@/lib/leitura/grifos'

const useIso = typeof window !== 'undefined' ? useLayoutEffect : useEffect
const rotuloGrifo = (t?: string | null) => (t && t in GRIFOS ? GRIFOS[t as TipoGrifo].label : 'Grifo')
...
export function LeituraAutorAnotacoes({ documentoId, versao, html }: { documentoId: string; versao: number; html: string }) {
  ...
  async function criar(tipo: TipoGrifo) {
    if (!sel) return
    const a = sel.anc; const cor = corDoGrifo(tipo)
    setSel(null); window.getSelection()?.removeAllRanges()
    const r = await criarAnotacaoBase(documentoId, versao, { inicio: a.inicio, fim: a.fim, exact: a.exact, prefix: a.prefix, suffix: a.suffix, cor, tipoGrifo: tipo })
    if (r.ok && r.id) setAnotacoes((p) => [...p, { id: r.id!, inicio: a.inicio, fim: a.fim, exact: a.exact, prefix: a.prefix, suffix: a.suffix, cor, nota: null, tipoGrifo: tipo }])
    else toast.error(r.error ?? 'Erro ao grifar.')
  }
  async function trocarTipo(id: string, tipo: TipoGrifo) {
    setAnotacoes((p) => p.map((a) => (a.id === id ? { ...a, tipoGrifo: tipo, cor: corDoGrifo(tipo) } : a)))
    await atualizarAnotacaoBase(id, { tipoGrifo: tipo, cor: corDoGrifo(tipo) })
  }
  async function atualizar(id: string, patch: Partial<Pick<AnotacaoBase, 'nota'>>) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
      ...
      <div ref={containerRef} className="relative rounded-2xl border bg-card">
        <div className="flex items-center gap-2 border-b px-4 py-2 text-sm font-medium"><Highlighter className="h-4 w-4 text-primary" /> Selecione um trecho e escolha o tipo de grifo (vem no documento p/ todos)</div>
        ...
        {sel && (
          <div className="absolute z-30 -translate-x-1/2 rounded-xl border bg-popover p-1.5 shadow-lg" style={{ left: sel.x, top: sel.y }} onMouseDown={(e) => e.preventDefault()}>
            <div className="grid grid-cols-3 gap-1">
              {GRIFOS_LISTA.map((gr) => (
                <button key={gr.id} onClick={() => criar(gr.id)} className="inline-flex items-center gap-1 rounded-md px-1.5 py-1 text-[11px] font-medium transition hover:bg-muted" title={gr.label}>
                  <span className="h-3 w-3 shrink-0 rounded-full border border-black/10" style={{ background: gr.cor }} /> {gr.label}
                </button>
              ))}
              <button onClick={() => { setSel(null); window.getSelection()?.removeAllRanges() }} className="col-span-3 mt-0.5 inline-flex items-center justify-center rounded-md py-0.5 text-xs text-muted-foreground hover:bg-muted"><X className="mr-1 h-3.5 w-3.5" /> Cancelar</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl border bg-card">
        <div className="border-b px-3 py-2 text-sm font-semibold">Grifos editoriais ({anotacoes.length})</div>
          ...
          {anotacoes.length === 0 ? (
            ...
            <div key={a.id} className="rounded-lg border p-2">
              <div className="flex items-start gap-2">
                <span className="mt-0.5 h-3 w-3 shrink-0 rounded-full" style={{ background: corDoGrifo(a.tipoGrifo) }} title={rotuloGrifo(a.tipoGrifo)} />
                <button onClick={() => pular(a)} className="min-w-0 flex-1 text-left text-xs leading-snug">
                  <span className="mr-1 rounded bg-muted px-1 py-0.5 text-[9px] font-bold uppercase text-muted-foreground">{rotuloGrifo(a.tipoGrifo)}</span>
                  <span className="line-clamp-2 align-middle">{a.exact}</span>
                </button>
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-1 pl-5">
                {GRIFOS_LISTA.map((gr) => <button key={gr.id} onClick={() => trocarTipo(a.id, gr.id)} className={cn('h-4 w-4 rounded-full transition', a.tipoGrifo === gr.id ? 'ring-2 ring-primary' : 'border border-black/10')} style={{ background: gr.cor }} title={gr.label} />)}

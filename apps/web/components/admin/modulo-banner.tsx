...
export function ModuloBanner({ banner, titulo, subtitulo, topoDireita, breadcrumb, tabs }: {
  ...
  useEffect(() => {
    ...
    if (!el || !root) return
    // Compacto quando a sentinela (no topo, fora do sticky) sai da viewport → rolou para baixo.
    const io = new IntersectionObserver(([e]) => setCompacto(!e.isIntersecting), { root, rootMargin: '-8px 0px 0px 0px' })
    io.observe(el)
  ...
  return (
    <>
      ...
      <div
        className="sticky -top-6 z-30 -mx-6 -mt-6 flex flex-col overflow-hidden bg-neutral-950 transition-[min-height] duration-300 ease-out"
      ...
      >
        ...
        <div className="relative flex flex-1 flex-col px-6 pt-4 text-white">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                ...
                {/* Subtítulo — some (altura + opacidade) no modo compacto. */}
                <div className={cn('grid transition-all duration-300 ease-out', compacto ? 'grid-rows-[0fr] opacity-0' : 'grid-rows-[1fr] opacity-100')}>
          ...
          <div className={cn('grid transition-all duration-300 ease-out', compacto ? 'grid-rows-[0fr] opacity-0' : 'mt-1.5 grid-rows-[1fr] opacity-100')}>

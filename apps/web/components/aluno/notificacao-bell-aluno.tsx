 ...
 *  O painel abre em PORTAL, ancorado ao sino (que vive no rodapé da sidebar), abrindo para cima. */
    ...
    <>
      ...
      {aberto && pos && typeof document !== 'undefined' && createPortal(
        <div
          ...
          className="fixed z-[120] flex max-h-[75vh] w-[340px] flex-col overflow-hidden rounded-2xl border border-white/10 shadow-2xl"
        >
          <header className="flex items-center justify-between border-b border-white/10 px-5 py-4">
            <span className="text-base font-bold text-white">Notificações</span>
            <button onClick={() => setAberto(false)} aria-label="Fechar" className="rounded-md p-1 text-sidebar-foreground/60 transition-colors hover:bg-white/10 hover:text-white"><X className="h-4 w-4" /></button>
          </header>
          ...
          <div className="min-h-0 flex-1 overflow-auto">
            {items.length === 0 ? (
              ...
              items.map((i) => (
                <button key={i.id} onClick={() => abrir(i)} className={cn('flex w-full gap-3 border-b border-white/[0.06] px-5 py-4 text-left transition-colors last:border-0 hover:bg-white/5', i.link && 'cursor-pointer')}>

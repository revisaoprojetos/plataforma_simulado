...
export function PersonalizadoEditor({ simuladoId, titulo: tituloIni, itensIniciais, secoesIniciais, visualInicial, tempoInicial, voltarUrl = '/aluno/simulados' }: {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border bg-card p-4">
        <div className="mb-2.5 flex items-center gap-2">
          <span className="text-sm font-semibold">Aparência do card</span>
          {salvandoVisual && <span className="inline-flex items-center gap-1 text-xs font-normal text-muted-foreground"><Loader2 className="h-3 w-3 animate-spin" /> salvando</span>}
        </div>
        <VisualPersonalizadoPicker cor={visual.cor} icone={visual.icone} onChange={mudarVisual} titulo={null} />
      </div>

 ...
 *  com setas no hover e bolinhas. Molde FIXO 1920×500; imagem em object-cover (preenche
 *  o molde, corta o excesso — sem esticar e sem barras). O recorte controla o enquadramento. */
function FaixaRotativa({ banners }: { banners: BannerPortal[] }) {
  ...
  const conteudo = b.imagem_url ? (
    // eslint-disable-next-line @next/next/no-img-element
    <img key={b.id} src={b.imagem_url} alt={b.titulo ?? ''} className="aspect-[1920/500] w-full animate-in fade-in object-cover duration-700" />
  ) : (

...
const canon=(l)=>{const s=l.toLowerCase();
  if(s.includes('9.307')||s.includes('arbitragem'))return 'Lei de Arbitragem (9.307/96)';
  if(s.includes('5.172')||s.includes('tributário nacional')||s.includes('ctn'))return 'CTN (5.172/66)';
  if(s.includes('responsabilidade fiscal')||s.includes('101/2000'))return 'LRF (LC 101/00)';
  return l.trim();}
const rows = parseCSV(raw); const head=rows[0].map(h=>h.trim())
const leiCols=['Lei A','Lei B','Lei C','Lei D','Lei E'].map(h=>head.indexOf(h))
const data=rows.filter(r=>/^\d+$/.test((r[0]||'').trim()))
const dist=[]
for(const r of data){const set=new Set()
  for(const ci of leiCols){const v=(r[ci]||'').trim();if(!v)continue;v.split(';').map(s=>s.trim()).filter(Boolean).forEach(l=>set.add(canon(l)))}
  dist.push({n:+r[0].trim(),disc:(r[head.indexOf('Disciplina')]||'').trim(),leis:[...set]})}
const c2=dist.filter(d=>d.leis.length===2),c3=dist.filter(d=>d.leis.length>=3)
console.log(`── 2 leis distintas: ${c2.length} questões ──`)
c2.forEach(d=>console.log(`  Q${d.n} [${d.disc}]: ${d.leis.join(' + ')}`))
console.log(`\n── 3+ leis distintas: ${c3.length} questões ──`)
